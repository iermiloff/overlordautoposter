...
def add_post(media_type, media_id, text, buttons, interval=None, publish_at=None, is_delayed=0, target_channels=None):
    ...
    # Гарантируем, что buttons — это строка JSON перед записью
    if isinstance(buttons, (list, dict)):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        cursor.execute(
            ...
            (media_type, media_id, text, buttons_json, interval, publish_at, is_delayed, json.dumps(target_channels))
        )
        conn.commit()
